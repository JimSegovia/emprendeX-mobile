...
export default function ProductosScreen() {
  ...
  const filteredItems = useMemo(() => {
    ...
    return CATALOG_ITEMS.filter((item) => {
      ...
      if (!q) return true;
      const hay =
        `${item.name} ${item.description} ${item.id} ${item.sku ?? ""}`.toLowerCase();
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <View>
            ...
            <Animated.View className="mb-2" entering={sectionEntering(1)}>
              <ScrollView
                horizontal
              ...
              >
                <View className="flex-row">
                  <TouchableOpacity
                    className="mr-3 flex-row items-center rounded-2xl bg-violet-100 px-3.5 py-2.5"
                    onPress={() => {
                      setFilter((prev) =>
                        prev === "all"
                          ? "Producto"
                  ...
                  <TouchableOpacity
                    className="flex-row items-center rounded-2xl bg-violet-600 px-4 py-2.5"
                    activeOpacity={0.85}
                    onPress={() => {
